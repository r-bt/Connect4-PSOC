...
#define X_Min               400     // Min and Max coordinates in raw values for calibration
#define X_Max               3500
#define Y_Min               400
#define Y_Max               3700
...
uint8 XPT2046_ReadXY(uint16 *X,  uint16 *Y)  // Return 1 if data is valid
{
    ...
    { 
        ...
        px = ((xav - X_Min) * XSIZE_PHYS) / (X_Max - X_Min); // Calibration for LCD Panel Size; needs Max & Min data Raw for each coordinate
        if(px <= 0) px = 0;
        if(px >= XSIZE_PHYS) px = XSIZE_PHYS;
        py = ((yav - Y_Min) * YSIZE_PHYS) / (Y_Max - Y_Min);
        if(py <= 0) py = 0;
